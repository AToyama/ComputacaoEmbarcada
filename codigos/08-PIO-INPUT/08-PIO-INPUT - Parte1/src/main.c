...
*
* 08-PIO-ENTRADA
*Parte 1: Utilizar o botão reservado ao usuário para ativar o LED
*         (Botão não pressionado = LED acesso/ Botão pressionado = Led apagado)
...
int main(void)
{
	...
	while(1){
		...
		if((BUT_PIO->PIO_PDSR & BUT_PIN_MASK)==0){				//SE o botão do usuário está pressionado (0 -> chave fechada/1 -> chave aberta)
						LED_PIO->PIO_CODR = LED_PIN_MASK;	    //Ativa o LED      
		}
		else{
			LED_PIO->PIO_SODR = LED_PIN_MASK;	//senão, apaga o led
		}
